...
def run_comprehensive_latency_test(
    index_name: str, namespace: str = "", test_sizes: List[int] = None
):
    ...
    for size in test_sizes:
        if size > total_vectors:
            print(
                f"--- Skipping top_k={size} (only {total_vectors} vectors available) ---"
            )
            continue
        ...
        size_reduction_pct = (
            ((full_response_size - meta_response_size) / full_response_size * 100)
            if full_response_size > 0
            else 0
        )

        # Estimate vector data size (rough approximation)
        if meta_matches > 0:
            vector_size_per_item = (
                full_response_size - meta_response_size
            ) / meta_matches
            estimated_vector_bytes = (
                vector_size_per_item * dimension * 4
            )  # 4 bytes per float32
        else:
    ...
    if results:
        ...
        print(".1f")
        print(
            f"Average matches returned: {sum(r['matches'] for r in results) / len(results):.0f}"
        )
...
def main():
    ...
    parser.add_argument(
        "--batch-size", type=int, default=100, help="Batch size for fetch tests"
    )
    parser.add_argument(
        "--query-only", action="store_true", help="Only run query tests"
    )
    parser.add_argument(
        "--fetch-only", action="store_true", help="Only run fetch tests"
    )
