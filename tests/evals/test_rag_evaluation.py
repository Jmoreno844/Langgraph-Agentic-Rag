...
@pytest.mark.parametrize(
    "test_case_data", load_evaluation_dataset()[:2]
)  # Test first 2 cases for demo
async def test_rag_evaluation(test_case_data: dict):
    ...
    try:
        final_state = await graph_app.ainvoke(
            {"messages": [("user", input_question)]}, config
        )
    ...
    metrics_to_evaluate = [
        FaithfulnessMetric(
            threshold=MIN_SCORE_THRESHOLD, model="gpt-4o-mini", include_reason=True
        ),
        AnswerRelevancyMetric(
            threshold=MIN_SCORE_THRESHOLD, model="gpt-4o-mini", include_reason=True
        ),
...
def test_dataset_integrity():
    ...
    for i, test_case in enumerate(test_cases):
        ...
        assert (
            "expected_output" in test_case
        ), f"Test case {i} missing 'expected_output' field"
        ...
        assert (
            len(test_case["expected_output"].strip()) > 0
        ), f"Test case {i} has empty expected_output"
